...
class Captcha(object):
       ...
       '''
        ...
        def entry(self):
            # 验证码图片的宽和高
            self.imageW = self.imageW if self.imageW else self.length*self.fontSize*1.2+self.length*self.fontSize/2
            self.imageH = self.imageH if self.imageH else self.fontSize * 2.5
            ...
            if self.useCurve:
                for x in range(0, self.useCurve):
                    self.__gene_line(draw, self.imageW, self.imageH)
            # # 绘制杂点
            if self.useNoise:
